/*
 * Delete Middle Node
 * Delete the node given pointer only to that 
 * Prereq: LinkedList Basics
 */
#include <stdio.h>
#include <stdlib.h>
 ...
 * The logic is to get the value from the next node
 * onto the current node and update the pointer to point 
 * to next->next node
 */
void DeleteMiddleNode(struct node* node) {
  if(node->next == NULL) {
    printf("Can't be done for the last node\n");
    return;
  }
...
int main() {
  ...
  middle = head->next->next->next->next;
